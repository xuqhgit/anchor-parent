...
public class RoleController extends BaseController {
    ...
    @ResponseBody
    public Result permission(@PathVariable("id") long id, Long[] permissionIds) {
        try {
            long count = roleService.saveRolePermission(id, Arrays.asList(permissionIds), TokenManager.getToken().getId());
            return new ResultObject().setData(count);
        } catch (Exception e) {
            e.printStackTrace();
            return new Result().error("保存角色权限失败：" + e.getMessage());
        }
    }
